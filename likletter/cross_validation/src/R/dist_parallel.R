Sys.setenv(
  PATH = paste(Sys.getenv("PATH"), "C:/rtools45/mingw64/bin/", sep = ";")
)
Sys.setenv(BINPREF = "C:/rtools45/mingw64/bin/")

cv_parallel <- function(
  Y,
  X,
  K_range,
  m,
  ...
  cv <- foreach(i = 1:nrow(KL), .inorder = FALSE, .packages = c("Rcpp")) %dopar%
    {

      source(paste0(loc, "cv/src/R/dist_functions.R"))
      sourceCpp(paste0(loc, "cv/src/cpp/dist_functions.cpp"))
