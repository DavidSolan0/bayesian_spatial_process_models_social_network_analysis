...
read_chain_file <- function(path_outs, prefix, param_name, n_sams, n_cols) {
    file_path <- paste0(path_outs, prefix, "_", param_name, "_chain.txt")
    data <- read.table(file_path, quote = "\"", comment.char = "")
    matrix(c(as.matrix(data)), n_sams, n_cols, TRUE)
}

adjacency.plot <- function(
    mat,
    show.grid = FALSE,
    cex.axis,
    tick,
...
data_gen2 <- function(
    ...
    if (plotit) {
        ...
        net.data <- graph_from_adjacency_matrix(
            Y,
            diag = F,
            mode = "undirected"
        )
    ...
    return(list(z = z, Y = Y, Theta = Theta, graph = graph, surface = fig))
}
